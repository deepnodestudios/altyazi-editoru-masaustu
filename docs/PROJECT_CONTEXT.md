...
- **Batch Translate (Toplu Çeviri):** Bu özellik **Ağustos 2026'da koddan tamamen kaldırılmıştır.** Frontend (controller, servisler, UI, ayarlar) ve backend (`startBatchTranslation`, `checkBatchTranslation`, `pollBatchJobs` fonksiyonları, `batchTranslate.ts`, `pollBatchJobs.ts`) dahil tüm kod silinmiştir. Not: Çoklu dosya seçilerek sıralı çeviri yapan **istemci kuyruğu** (`startBatchTranslationQueue`) aktif bir özelliktir ve korunmalıdır; yalnızca Gemini Batch API tabanlı bulut batch özelliği kaldırılmıştır.
